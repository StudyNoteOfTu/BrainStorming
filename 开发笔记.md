...
例如在Retrofit中，可以**通过在CallAdapter这里进行功能拓展**，对请求结果的适配过程中做一层处理，将各种异常情况进行逻辑判断，并视情况返回上述的NetworkResponse子类，从而实现 **适配与功能拓展**。在onResponse中如果逻辑判断为异常，则再转发到onException回调，交还给上层。（示例逻辑判断如下：例如200...300的结果码，就认为是成功，而404这样的结果码不应该是成功，但不会报错，逻辑上应该判定为失败的，所以在功能拓展这一层，转发给onException）
...
1. 例子1
   ...
   不同的请求头，根据不同的baseUrl，配置不同的请求头，一层封装
